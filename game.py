...
def get_pawn_moves(team_board : np.array, team_board_pop : np.array, opp_board_pop : np.array, team : int):
    ...
    for pawn_pos in pawn_poss:
        ...
        for candidate_pos, candidate_diff in zip(candidate_poss, candidate_diffs):
            ...
            # true if there's no pin in any direction, in which case the candidate move is added to the results list
            if not np.all([forward[:2] == [team, KING] and backward[:2] in [[int(not team), t] for t in types]
                                or backward[:2] == [team, KING] and forward[:2] in [[int(not team), t] for t in types]
...
def get_queen_moves(board : np.array, team_board : np.array, team : int):
    queen_moves = []
