...
class TestMoves(unittest.TestCase):
    ...
    def test_king_moves(self):
        for king_coord in np.array([[rank, file] for rank in range(8) for file in range(8)]):
            ...
            self.assertTrue(contains_exactly(true_moves, moves[king_piece]))
    ...
    def test_king_moves_controlled(self):
        ...
        self.assertTrue(contains_exactly(true_moves, moves[king_piece]))
        self.assertTrue(contains_exactly([Move([3, 7])], moves[pawn_piece]))
    ...
    def test_king_moves_blocked(self):
        ...
        self.assertEqual(len(moves[king_piece]), len(true_moves))
        self.assertTrue(contains_exactly(moves[king_piece], true_moves))
    ...
    def test_pawn_moves(self):
        for team, ranks in zip([WHITE, BLACK], [[6, 5, 4], [1, 2, 3]]):
            for file in range(8):
                ...
                self.assertTrue(contains_exactly(moves[pawn_piece], true_moves))

                board.add_piece(PAWN, other_team(team), ranks[2], file)
                moves = get_moves(board, team)
                self.assertTrue(contains_exactly(moves[pawn_piece], true_moves[:1]))
